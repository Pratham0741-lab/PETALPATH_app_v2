...
const LeftHill = React.memo(({ top, width }: { top: number; width: number }) => (
  <View style={{ position: 'absolute', top, left: 0, width: width * 0.45, height: 220, zIndex: -2 }}>
...
const RightHill = React.memo(({ top, width }: { top: number; width: number }) => (
  <View style={{ position: 'absolute', top, right: 0, width: width * 0.45, height: 220, zIndex: -2 }}>
    <Svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
...
const GardenTree = React.memo(({ top, left }: { top: number; left: number }) => (
  <View style={{ position: 'absolute', top, left, width: 50, height: 60, zIndex: -2 }}>
    <Svg width="50" height="60" viewBox="0 0 50 60">
      <Rect x="23" y="35" width="5.5" height="25" rx="1.5" fill="#8C6F5A" />
      <Circle cx="25" cy="22" r="18" fill="#8DBB75" />
      <Circle cx="17" cy="16" r="10" fill="#9CD184" />
      <Circle cx="18" cy="14" r="3" fill="#F6B5C5" />
      <Circle cx="32" cy="25" r="3" fill="#F6B5C5" />
    </Svg>
  </View>
));

const GardenBush = React.memo(({ top, left }: { top: number; left: number }) => (
  <View style={{ position: 'absolute', top, left, width: 45, height: 28, zIndex: -2 }}>
    <Svg width="45" height="28" viewBox="0 0 45 28">
      <Path d="M 4 28 C 0 9, 16 4, 22 15 C 29 4, 45 9, 41 28 Z" fill="#9CD184" />
      <Circle cx="13" cy="18" r="2.5" fill="#FFF" />
      <Circle cx="28" cy="15" r="2.5" fill="#FFF" />
    </Svg>
  </View>
));

const TinyFlower = React.memo(({ top, left, color = '#F6B5C5' }: { top: number; left: number; color?: string }) => (
  <View style={{ position: 'absolute', top, left, width: 18, height: 22, zIndex: -2 }}>
    <Svg width="18" height="22" viewBox="0 0 18 22">
      <Path d="M9 10 L9 22" stroke="#7CA767" strokeWidth="1.5" />
      <Circle cx="9" cy="8" r="4.5" fill={color} />
      <Circle cx="4.5" cy="8" r="3.5" fill={color} />
      <Circle cx="13.5" cy="8" r="3.5" fill={color} />
      <Circle cx="9" cy="4.5" r="3.5" fill={color} />
      <Circle cx="9" cy="8" r="2" fill="#F7C94B" />
    </Svg>
  </View>
));
  ...
  return (
    ...
    >
      <Svg width="16" height="9" viewBox="0 0 16 9">
        <Path d="M 0 4.5 C 3 0, 11 0, 16 4.5 C 11 9, 3 9, 0 4.5 Z" fill="#F6B5C5" opacity={0.8} />
      </Svg>
    </Animated.View>
  );
});
...
export const HomeDesktop: React.FC = () => {
  ...
  const screenWidth = Math.min(windowWidth, 768); // Center with max width on large screens
    ...
    if (index % 3 === 0) {
      rowDecoration = (
        <>
          ...
          <GardenTree top={hillTop + 40} left={25} />
          <TinyFlower top={hillTop + 120} left={70} color="#F6B5C5" />
          <TinyFlower top={hillTop + 140} left={95} color="#B89DE8" />
          <FloatingPetal top={hillTop + 20} left={pathCenterX + 30} delay={index * 300} />
        </>
      );
    } else if (index % 3 === 1) {
      rowDecoration = (
        <>
          ...
          <GardenBush top={hillTop + 55} left={screenWidth - 85} />
          <TinyFlower top={hillTop + 110} left={screenWidth - 105} color="#F29A8F" />
          <FloatingPetal top={hillTop + 160} left={pathCenterX - 45} delay={index * 300} />
        </>
      );
    } else {
      rowDecoration = (
  ...
  return (
    <ScreenContainer>
      <View style={styles.root}>
        {/* CUSTOM TOP BAR (Centered on Desktop View) */}
