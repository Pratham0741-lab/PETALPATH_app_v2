...
export class WriteProgressRepository {
  async findProgress(childId: string, activityId: string) {
    return prisma.writeProgress.findUnique({
      where: {
        childId_activityId: {
          childId,
          activityId,
  ...
  async upsertProgress(childId: string, activityId: string, data: { isCompleted: boolean; score?: number; attemptCount?: number }) {
    ...
    const bestStars = starService.calculateWriteStars(bestScore);

    return prisma.writeProgress.upsert({
      where: {
        childId_activityId: {
          childId,
          activityId,
...
export const writeProgressRepository = new WriteProgressRepository();
