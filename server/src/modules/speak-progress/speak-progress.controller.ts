...
import { speakProgressService } from './speak-progress.service.js';
import { ValidationError, UnauthorizedError } from '../../utils/errors.js';

export class SpeakProgressController {
  async getProgress(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const progress = await speakProgressService.getProgress(childId, activityId);
  ...
  async saveProgress(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const progress = await speakProgressService.saveProgress(
        childId,
        activityId,
        Boolean(isCompleted),
  ...
  async completeSpeak(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const childId = req.user?.childId;
      if (!childId) {
        throw new UnauthorizedError('Active child profile is not selected');
      ...
      const progress = await speakProgressService.completeSpeak(
        childId,
        activityId,
        score !== undefined ? Number(score) : undefined
      );
...
export const speakProgressController = new SpeakProgressController();
